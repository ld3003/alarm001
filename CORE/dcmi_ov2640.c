...
void OV2640_CaptureGpioInit(void)
{
  ...
  NVIC_InitTypeDef NVIC_InitStructure;
	...
	/* Configure Button EXTI line */
	EXTI_InitStructure.EXTI_Line = /*EXTI_Line8 | EXTI_Line13 | */ EXTI_Line15;
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Rising;
	EXTI_InitStructure.EXTI_LineCmd = ENABLE;
	EXTI_Init(&EXTI_InitStructure);
...
void readimgx(void)
{
	...
	//可以默认是low1，也可以默认是low3
	unsigned char READ_VSYNC_WAIT_STATUS	= OV_READ_IMG_VSYNC_LOW1;
	unsigned int STATUS_TIM = 0; //记录在一个状态停留的时间
	unsigned char READSTATSUS = OV_READ_IMG_INTI;
	unsigned char READSTATSUS_HIS = OV_READ_IMG_NULL;
	...
	for(;;)
	{
		...
		switch(READSTATSUS)
		{
			case OV_READ_IMG_INTI:
			{
				//等到VSYNC拉低的时候
				JpegDataCnt = 0;
				if (READ_VSYNC == 0)
				{
					READSTATSUS = READ_VSYNC_WAIT_STATUS; //OV_READ_IMG_VSYNC_LOW3,OV_READ_IMG_VSYNC_LOW1
				}
				break;
			}
			
			case OV_READ_IMG_VSYNC_LOW1:
			{
				//现在已经拉低,再等一次拉高
				JpegDataCnt = 0;
				if (READ_VSYNC != 0)
					READSTATSUS = OV_READ_IMG_VSYNC_LOW2;
				break;
			}
			case OV_READ_IMG_VSYNC_LOW2:
			{
				//现在已经拉高了，再等一次拉低
				if (READ_VSYNC == 0)
					READSTATSUS = OV_READ_IMG_VSYNC_LOW3;
				break;
			}
			case OV_READ_IMG_VSYNC_LOW3:
			{
				//第二次被拉低，下面可以继续等待拉高后就可以读取图片了，等待两次是因为怕第一次时间太短
				if (READ_VSYNC != 0)
					READSTATSUS = OV_READ_IMG_VSYNC_HIGH;
				break;
			}
			case OV_READ_IMG_VSYNC_HIGH:
			{
				//__OV_READ_IMG_VSYNC_HIGH:
				if (READ_VSYNC != 0)
				{
					...
					if ((READ_PICLK != 0) && (READ_HREF != 0))
					{
						...
						READSTATSUS = OV_READ_IMG_WAIT_PICLK;
						//goto __OV_READ_IMG_WAIT_PICLK;
					}
					//
				}else {
					READSTATSUS = OV_READ_IMG_FINISH;
				}
				break;
			}
			
			case OV_READ_IMG_WAIT_PICLK:
			{
				//__OV_READ_IMG_WAIT_PICLK:
				//((READ_PICLK != 0) && (READ_HREF != 0))
				//if ((READ_VSYNC != 0) && (READ_PICLK != 0))//
				if ((READ_VSYNC != 0) && ((READ_PICLK == 0) || (READ_HREF == 0) ))
				{
					READSTATSUS = OV_READ_IMG_VSYNC_HIGH;
					//goto __OV_READ_IMG_VSYNC_HIGH;
				}
				
				break;
			}
			case OV_READ_IMG_FINISH:
			{
				int i=0;
...
unsigned char XOV2640_Init[][2]=
{
    ...
    0x12,   0x80,  
    0x3c,   0x35,  
    0x11,   0x00,//0x02,  //BIT[5:0] CLK=XVCLK/(bit[5:0]+1)
    0x04,   0x28,  
    0x13,   0xe5,  
    0x14,   0x48,  
    0x2c,   0x0c,  
    ...
    0x50,   0x9c,  
    0x6d,   0x80,  
    0x39,   0x02,	//0X39,0X82,????1.3V LDO // 0X39,0X02,//????LDO
    0x3a,   0x33,  
    0x3b,   0xfb,  
    0x3c,   0x32, 
 
    0x12,   0x40,  	// set sensor to SVGA  
    0x17,   0x11,     
    0x18,   0x43,     
    0x19,   0x00,     
    0x1a,   0x4b,     
	...
	0xe0,   0x05,   	// add the following 4 line to set  YUV output 
    0x86,   0x3d,     
    0xc0,   0x64,//64,       // image horizontal size HSIze[10:3]  
    0xc1,   0x4b,//4b,       // image Vertical size VSIze[10:3]  
    0x8c,   0x00,       // HSize[11],HSize[2:0],Vsize[2:0]  
    0x50,   0x89,       // down sample /4  ???
    0x51,   0xc8,//C8,		// H_Size[7:0], (real/4)  
    0x52,   0x96,//96,		// V_Size[7:0]  (real/4)  
    ...
    0xd3,   0x7f,  		// bit 6..0,DVP PCLK=sysclk(48)/[6..0]  
    0xe1,   0x67,   
...
{
	int i=0;
	for(i=0; i<(sizeof(XOV2640_Init)/2); i++)
  {
    OV2640_WriteReg(XOV2640_Init[i][0], XOV2640_Init[i][1]);
		rt_thread_sleep(1);
  }
	
	for(i=0; i<(sizeof(XOV2640_JPEG)/2); i++)
  {
    OV2640_WriteReg(XOV2640_JPEG[i][0], XOV2640_JPEG[i][1]);
		rt_thread_sleep(1);
  }
	//
}
...
static void PWM_GPIO_Configuration(void)  
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;//GPIO_Pin_3??channel3,?????:TIM_SetCompare3 
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
...
static void TIM1_Configuration(void)  
{
  ...
  TIM_OCInitTypeDef TIM_OCInitStructure;
	...
	memset(&TIM_OCInitStructure,0x0,sizeof(TIM_OCInitStructure));
	memset(&TIM_TimeBaseStructure,0x0,sizeof(TIM_TimeBaseStructure));
  
  TIM_DeInit(TIM1);
  TIM_InternalClockConfig(TIM1);
  //??????
  TIM_TimeBaseStructure.TIM_Period = 10-1;//????100,2MHz=2000KHz,2MHz/100=0.02MHz,????T=1/0.02MHz=50us,??????????50us
  TIM_TimeBaseStructure.TIM_Prescaler = 2-1;//36???,72MHz/36=2MHz
  ...
  TIM_OCInitStructure.TIM_Pulse = 50;//?????50,??????:50/100=50%
...
int configpwm(void)
{
  ...
  /* Compute the value to be set in ARR regiter to generate signal frequency at 17.57 Khz */
	//48000000
  TimerPeriod = (SystemCoreClock / 3000000 ) - 1;
...
void ov2640_read(void)
{
	...
	//memcpy(JpegBuffer,ximgbuffer,sizeof(ximgbuffer));
	//JpegBuffer = (char*)ximgbuffer;
	//JpegDataCnt = sizeof(ximgbuffer);
	
	//check jpg
	//load_jpg(0,Buff,sizeof(Buff));
